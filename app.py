...
st.set_page_config(page_title="CalcJus Pro 5.0 (Lei 14.905/24)", layout="wide", page_icon="⚖️")
...
def gerar_pdf_relatorio(dados_ind, dados_hon, dados_pen, dados_aluguel, totais, config):
    ...
    if "Lei 14.905" in tipo_regime:
        texto_explicativo += (
            "METODOLOGIA APLICADA (Nova Lei n. 14.905/2024):\n"
            "1. FASE INICIAL (Até 29/08/2024): Incidência da Taxa SELIC (Coincidência de Correção e Juros).\n"
            "2. FASE ATUAL (A partir de 30/08/2024): Atualização Monetária pelo IPCA e Juros pela Taxa Legal (SELIC - IPCA), conforme art. 406 do CC e Resolução CMN 5.171/2024."
        )
    elif "Misto" in tipo_regime:
        ...
        texto_explicativo += (f"METODOLOGIA APLICADA (Regime Misto - EC 113/21):\n1. FASE PRÉ-SELIC (Até {dt_corte}): Correção monetária pelo índice original{com_juros_txt}.\n2. FASE SELIC (De {dt_corte} até {dt_calc}): A Taxa SELIC incidiu exclusivamente sobre o PRINCIPAL CORRIGIDO.")
    elif "SELIC" in tipo_regime:
        texto_explicativo += "METODOLOGIA APLICADA: Taxa SELIC Pura (Correção + Juros em fator único)."
    else:
        texto_explicativo += "METODOLOGIA APLICADA (Padrão): Correção Monetária plena + Juros de Mora de 1% a.m."
    ...
    if not dados_ind.empty:
        ...
        # Define Headers Baseados no Regime
        if "Lei 14.905" in tipo_regime or "SELIC" in tipo_regime:
             headers = [("Vencimento", 30), ("Valor Orig.", 35), ("Fator SELIC/Legal", 50), ("TOTAL", 40)]
...
# NAVEGAÇÃO
# ==============================================================================
st.title("⚖️ CalcJus PRO 5.0 - Lei 14.905/2024")
st.markdown("Cálculos Judiciais com Precisão Decimal, API BCB e Suporte à Nova Lei.")
...
with tab1:
    ...
    valor_contrato = to_decimal(col_i1.number_input("Valor Base (R$)", value=2119.04, step=100.00))
    ...
    c4, c5 = st.columns(2)
    # Exemplo: descontos ocorreram em 2024
    inicio_atraso = c4.date_input("Início (1º Desconto)", value=date(2024, 1, 15), format="DD/MM/YYYY")
    fim_atraso = c5.date_input("Fim (Último Desconto)", value=date(2024, 1, 15), format="DD/MM/YYYY")
    
    # --- NOVO MENU DE REGIMES ---
    regime_tipo = st.radio(
        "Regime de Atualização:",
        [
            "1. Índice Correção + Juros 1% a.m.", 
            "2. Taxa SELIC Pura (EC 113/21)", 
            "3. Misto (Índice até Corte -> SELIC)",
            "4. Lei 14.905/2024 (Transição em 30/08/2024)"
        ],
        horizontal=True
    )
    ...
    elif "Lei 14.905" in regime_tipo:
        st.info("ℹ️ Este modo aplica a SELIC integralmente. Matematicamente, IPCA + (SELIC - IPCA) = SELIC. O relatório detalhará a transição legal de 30/08/2024.")
        desc_regime_txt = "Nova Lei 14.905/2024"
    ...
    if st.button("Calcular Indenização", type="primary"):
        ...
        with st.status("Processando dados (Pro-Rata)...", expanded=True) as status:
            ...
            if "SELIC" in regime_tipo or "Misto" in regime_tipo or "Lei 14.905" in regime_tipo:
            ...
            for item in datas_calc:
                ...
                # 2. SELIC PURA OU NOVA LEI
                elif "2. Taxa SELIC" in regime_tipo or "Lei 14.905" in regime_tipo:
